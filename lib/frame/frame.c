...
static inline void insert_p(void)
{
    tx.data <<= 1;
    tx.data |= 0x01;
}
static inline void insert_s(void) { tx.data <<= 1; }
static inline void insert_ps(void)
{
    insert_p();
    insert_s();
}
...
{
    gpio_isr_handler_remove(CC_GDO0_GPIO);
...
static void tx_fifo_prime(void)
{
    ...
    txBits = 0;
    while (!gpio_get_level(CC_GDO0_GPIO))
        tx_fifo_send_block();
}
...
static void tx_fifo_fill(void)
{
    ...
    if (done) {
        ...
        gpio_set_intr_type(CC_GDO0_GPIO, GPIO_INTR_POSEDGE);
    }
}
...
{
    gpio_intr_disable(CC_GDO0_GPIO);
    xQueueSendFromISR(tx_isr_queue, NULL, NULL);
}

static void tx_fifo_init(void)
{
    gpio_reset_pin(CC_GDO0_GPIO); // Disconnect GDO0 from UART TX
    gpio_set_direction(CC_GDO0_GPIO, GPIO_MODE_INPUT);
    gpio_pulldown_en(CC_GDO0_GPIO);
    gpio_pullup_dis(CC_GDO0_GPIO);
...
static void tx_fifo_start(void)
{
    ...
    gpio_set_intr_type(CC_GDO0_GPIO, GPIO_INTR_NEGEDGE);

    tx_fifo_prime();
    tx_state = TX_FIFO_FILL;

    gpio_isr_handler_add(CC_GDO0_GPIO, GDO0_ISR, NULL);
}
...
void frame_init(void)
{
    ...
    esp_log_level_set(TAG, FRM_LOG_LEVEL);
...
void frame_work(void)
{
    ...
    case FRM_RX:
        ...
        if (rxFrm.state < FRM_RX_MESSAGE) { // RX not active
            ...
            if ((now - last_frm) > FRM_MIN_TX_DELAY) {
                ...
                if ((now - last_cal) > FRM_MAX_CAL_INTERVAL * 1000) {
