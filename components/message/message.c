...
static void msg_rx_process(uint8_t byte)
{
    ...
    case S_HEADER: {
        msgRx->state = msg_rx_header(msgRx, byte);
        break;
    }
    case S_ADDR0:
        if (msgRx->fields & F_ADDR0) {
            msgRx->state = msg_rx_addr(msgRx, 0, byte);
            break;
        } /* fallthrough */
    case S_ADDR1:
        if (msgRx->fields & F_ADDR1) {
            msgRx->state = msg_rx_addr(msgRx, 1, byte);
            break;
        } /* fallthrough */
    case S_ADDR2:
        if (msgRx->fields & F_ADDR2) {
            msgRx->state = msg_rx_addr(msgRx, 2, byte);
            break;
        } /* fallthrough */
    case S_PARAM0:
        if (msgRx->fields & F_PARAM0) {
            msgRx->state = msg_rx_param(msgRx, 0, byte);
            break;
        } /* fallthrough */
    case S_PARAM1:
        if (msgRx->fields & F_PARAM1) {
            msgRx->state = msg_rx_param(msgRx, 1, byte);
            break;
        } /* fallthrough */
    case S_OPCODE: {
        msgRx->state = msg_rx_opcode(msgRx, byte);
        break;
    }
    case S_LEN: {
        msgRx->state = msg_rx_len(msgRx, byte);
        break;
    }
    case S_PAYLOAD: {
        msgRx->state = msg_rx_payload(msgRx, byte);
        break;
    }
    case S_CHECKSUM: {
        msgRx->state = msg_rx_checksum(msgRx, byte);
        break;
    }
    }
}
...
uint8_t msg_scan(struct message* msg, uint8_t byte)
{
    ...
    if (!byte) {
        ...
        case S_HEADER:
            ok = msg_scan_header(msg, field, nChar);
            msg->state = S_PARAM0;
            break;
        case S_ADDR0:
            ok = msg_scan_addr(msg, field, nChar);
            msg->state = S_ADDR1;
            break;
        case S_ADDR1:
            ok = msg_scan_addr(msg, field, nChar);
        ...
        case S_ADDR2:
            ...
            break;
        case S_PARAM0:
            ok = msg_scan_param(msg, field, nChar);
            msg->state = S_ADDR0;
            break;
            //    case S_PARAM1:
        case S_OPCODE:
            ok = msg_scan_opcode(msg, field, nChar);
            msg->state = S_LEN;
            break;
        case S_LEN:
            ok = msg_scan_len(msg, field, nChar);
            msg->state = S_PAYLOAD;
            break;
        case S_PAYLOAD:
            msg->state = S_ERROR;
            break;
        case S_CHECKSUM:
            msg->state = (nChar != 1) ? S_ERROR : S_COMPLETE;
            break;
            //  case S_TRAILER:
            //  case S_COMPLETE:
            //  case S_ERROR:
...
static uint8_t msg_tx_process(struct message* msg, uint8_t* done)
{
    ...
    case S_START:
    case S_HEADER:
        byte = msg_tx_header(msg, &d);
        if (!d)
            break;
        msg->state = S_ADDR0; /* fall through */
    case S_ADDR0:
        byte = msg_tx_addr(msg, &d);
        if (!d)
            break;
        msg->state = S_ADDR1; /* fall through */
    case S_ADDR1:
        byte = msg_tx_addr(msg, &d);
        if (!d)
            break;
        msg->state = S_ADDR2; /* fall through */
    case S_ADDR2:
        byte = msg_tx_addr(msg, &d);
        if (!d)
            break;
        msg->state = S_PARAM0; /* fall through */
    case S_PARAM0:
        byte = msg_tx_param(msg, &d);
        if (!d)
            break;
        msg->state = S_PARAM1; /* fall through */
    case S_PARAM1:
        byte = msg_tx_param(msg, &d);
        if (!d)
            break;
        msg->state = S_OPCODE; /* fall through */
    case S_OPCODE:
        byte = msg_tx_opcode(msg, &d);
        if (!d)
            break;
        msg->state = S_LEN; /* fall through */
    case S_LEN:
        byte = msg_tx_len(msg, &d);
        if (!d)
            break;
        msg->state = S_PAYLOAD; /* fall through */
    case S_PAYLOAD:
        byte = msg_tx_payload(msg, &d);
        if (!d)
            break;
        msg->state = S_CHECKSUM; /* fall through */
    case S_CHECKSUM:
        byte = msg_tx_checksum(msg, &d);
        if (!d)
            break;
        msg->state = S_COMPLETE; /* fall through */
    case S_TRAILER:
    case S_COMPLETE:
    case S_ERROR:
        break;
...
#if 1
    if (!TxMsg) {
        ESP_LOGI(TAG, "msg_tx_get");
        struct message* tx1 = msg_tx_get();
        ESP_LOGI(TAG, "msg_tx_start");
        if (tx1)
            msg_tx_start(&tx1);
    }
#endif
